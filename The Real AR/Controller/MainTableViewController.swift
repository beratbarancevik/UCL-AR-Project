...
class MainTableViewController: UITableViewController {
	...
	func getAllScenes() {
		...
		ref.child("scenes").observeSingleEvent(of: .value, with: { (snapshot) in
			guard let scenesDictionary = snapshot.value as? NSDictionary else { print("E1"); self.showError(); return }
			
			// scenes loop
			for sceneItem in scenesDictionary {
				var steps = Array<Step>()
				guard let stepsDictionary = sceneItem.value as? NSDictionary else { print("E2"); self.showError(); return }
				
				// steps loop
				for stepItem in stepsDictionary {
					var virtualObjects = Array<VirtualObject>()
					guard let virtualObjectsDictionary = stepItem.value as? NSDictionary else { print("E3"); self.showError(); return }
					
					// virtual objects loop
					for virtualObjectItem in virtualObjectsDictionary {
						guard let coordinatesDictionary = virtualObjectItem.value as? NSDictionary else { print("E4"); self.showError(); return }
						...
						guard let virtualObject = VirtualObject(type: self.generateType(from: typeInt!), xCoor: xCoor!, yCoor: yCoor!, zCoor: zCoor!, text: text!) else { print("E5"); self.showError(); return }
						virtualObjects.append(virtualObject)
					}
					
					let stepNumber = stepItem.key
					guard let step = Step(stepNumber: stepNumber as! String, virtualObjects: virtualObjects) else { print("E6"); self.showError(); return }
					steps.append(step)
				}
				
				let sceneName = sceneItem.key
				guard let scene = Scene(name: sceneName as! String, steps: steps) else { print("E7"); self.showError(); return }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as! SceneCell
